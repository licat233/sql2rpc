 ...
 * @LastEditors: licat
 * @LastEditTime: 2023-02-17 12:59:46
...
func (s *ApiCore) Run() (err error) {
	...
	if config.C.ApiMultiple.GetBool() {
		...
		if filename == "" {
			filename = tools.ToSnake(config.C.DBSchema.GetString())
		}
		err = s.GenerateSingleFile(config.C.DBTable.GetString(), filename)
	}
	return
}
...
func (s *ApiCore) GenerateMultipleFile() error {
	...
	mainFilname := config.C.Filename.GetString()
	if mainFilname == "" {
		mainFilname = fmt.Sprintf("%s.api", tools.ToSnake(schema))
	}
	mainFilname = tools.SetFileType(mainFilname, ".api")
	...
	name := tools.GetFilename(mainFilname)
	for _, table := range tables {
		if _, ok := ignoreTables[table]; ok {
			continue
		}
		filename := fmt.Sprintf("%s_%s.api", schema, table)
		if filename == mainFilname {
			mainFilname = tools.FileRename(mainFilname, fmt.Sprintf("%s_%d", name, number))
			number++
		}
