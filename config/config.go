 ...
 * @LastEditors: licat
 * @LastEditTime: 2023-02-16 17:26:31
...
func NewDefaultConfig() *Config {
	return &Config{
		...
		Filename:        _item.NewConfigItem("filename", "", "the generated file name, defaults to the database schema", false),
		Pb:              _item.NewConfigItem("pb", false, "generate .proto files", false),
		PbPackage:       _item.NewConfigItem("pb_package", "", "the protocol buffer package, defaults to the database schema", false),
		PbGoPackage:     _item.NewConfigItem("pb_gopackage", "", "the protocol buffer go_package, defaults to the database schema", false),
...
func (c *Config) Initialize() {
	...
	if c.PbPackage.GetString() == "" {
		c.PbPackage.Set(tools.ToSnake(c.DBSchema.GetString()) + "_proto")
	}
	if c.PbGoPackage.GetString() == "" {
		c.PbGoPackage.Set("./" + tools.ToSnake(c.DBSchema.GetString()) + "_pb")
	}
	// if c.Filename.GetString() == "" {
	// 	c.Filename.Set(tools.ToSnake(c.DBSchema.GetString()))
	// }
